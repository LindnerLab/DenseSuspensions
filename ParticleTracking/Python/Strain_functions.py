...
def calc_strain(tracked, Cauchy=False, Hencky=False,
    ...
    if all(tracked[:nParticles].particle == tracked.particle.unique()):
        tracked = tracked.sort_values(by=['frame', 'particle'],
                                      ignore_index=True)
    elif not (all(tracked[:nParticles].frame == 0) and all(tracked.frame[list(range(0, nFrames * nParticles, nParticles))] == tracked.frame.unique())):
        tracked = tracked.sort_values(by=['frame', 'particle'],
                                      ignore_index=True)
    if x_max == np.nan:
...
def calc_strain_rate(tracked, Cauchy=False, Hencky=False,
                     dt=1, framerate=1, direction='x'):
